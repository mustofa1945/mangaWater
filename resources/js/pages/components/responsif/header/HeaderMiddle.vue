...
<template>
    <div :class="[isGenreStatusActive ? 'h-[91dvh]' : 'h-fit' , 'flex-col absolute  min-[576px]:flex left-0 top-10 w-fit  overflow-y-scroll bg-slate-900 py-2 min-[1200px]:hidden transition-all duration-300']">
        <div v-for="item in compuDropHeader.concatDrop.value" :class="`relative   text-gray-300 ${compuDropHeader.isDropActive.value ? 'w-[320px]' : 'w-[130px]' }`">
            <div v-if="item.isHaveDrop" class="w-full  flex flex-col">
                ...
                <div v-if="item.status" :class="[item.updateAnimeProper.height , 'w-full overflow-hidden transition-all duration-300 border-y-1 border-blue-600/50']">
