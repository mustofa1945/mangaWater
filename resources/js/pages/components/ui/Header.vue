...
<template>
    <nav v-bind="$attrs" v-showAndClose="readHeader.delayEffect"
        ...
        <div :class="`flex items-center gap-x-5`">
            <Link href="/home">
            <img alt="MangaFire.io logo" :class="`${menu.height} ${menu.widht} bg-cover`"
            ...
            <Link v-if="!menu.status" class="text-gray-400 hover:text-gray-100 transition-all duration-200 text-lg"
                href="#">
            Added
            </Link>
            <Link v-if="!menu.status" class="text-gray-400 hover:text-gray-100 transition-all duration-200 text-lg"
        ...
        <div class="relative flex items-center flex-1 mx-2">
            <i class="fa-sharp fa-solid fa-magnifying-glass absolute top-2.5 left-3 text-md text-gray-400"></i>
            <input class="px-4 py-2 pl-9 w-full rounded-full bg-gray-800 text-white text-sm focus:outline-none"
                placeholder="Search manga..." type="text" />
            <button class="bg-secondary text-white px-4 py-1 rounded-full text-sm absolute top-[0.6vh] right-[1vh]">
