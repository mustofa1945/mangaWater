...
<template>
    ...
    >
        <div
            :class="[
                !data.input ? 'justify-end' : '',
                'flex items-center bg-gray-800 px-3 py-2 ',
            ]"
        >
            <svg
                v-if="data.input"
        ...
        <div class="flex flex-col divide-y divide-gray-700">
            <div
                v-for="page in instanceProxy"
                @click="pickPage(page.id)"
                :class="[page.pageColor , 'relative w-full hover:bg-[#4169E1] saturate-70  cursor-pointer transition-all duration-100']"
            >
                <div
                    :class="[page.pageTranslateX , 'flex hover:translate-x-4 transition-all duration-500 items-center w-full py-2']"
                >
                    <i class="fa-solid fa-play text-sm"></i>
                    <span class="ml-2"> Page {{ page.id }} </span>
                </div>
            </div>
        </div>
    </div>
