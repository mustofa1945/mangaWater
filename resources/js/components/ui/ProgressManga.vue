...
<template>
    <div
        :class="[readTypePosition.style.property , 'Progress-bar bg-slate-800 z-20  justify-between  items-center absolute gap-1  duration-400']">
        <span v-if="readTypePosition.style.positionStatus" @click="pickPage(el.id)"
        ...
        <span v-if="!(readTypePosition.style.positionStatus)" @click="pickPage(el.id)"
            ...
            <span
                :class="[readTypePosition.style?.absolute , 'absolute px-5  w-full h-[1.5rem] rounded-md hidden group-hover:flex bg-[#4169E1] text-white justify-center items-center']">{{
                    el.id }}</span>
        </span>
